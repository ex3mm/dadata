...
/**
 * DTO для ответа Find Party API.
 */
final readonly class FindPartyResponse extends AbstractResponse
{
    /**
     * @param list<PartyDetailsDTO> $suggestions Список найденных организаций
        ...
        if (is_array($suggestionsData)) {
            foreach ($suggestionsData as $item) {
                if (is_array($item)) {
                    ...
                    $suggestions[] = PartyDetailsDTO::fromArray($item);
                }
            }
        }
