...
/**
 * Endpoint для поиска аффилированных компаний.
 */
final class FindAffiliatedEndpoint extends AbstractEndpoint
{
    protected function getPath(): string
    {
        return '/suggestions/api/' . self::API_VERSION . '/rs/findAffiliated/party';
    }
        ...
        if (is_array($suggestionsData)) {
            foreach ($suggestionsData as $item) {
                if (is_array($item)) {
                    ...
                    $items[] = AffiliatedPartyResponseDto::fromArray($item);
                }
            }
        }
