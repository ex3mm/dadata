...
/**
 * Endpoint для поиска банка по БИК, SWIFT, ИНН или регистрационному номеру.
 */
final class FindBankEndpoint extends AbstractEndpoint
{
    protected function getPath(): string
    {
        return '/suggestions/api/' . self::API_VERSION . '/rs/findById/bank';
    }
        ...
        if (is_array($suggestionsData)) {
            foreach ($suggestionsData as $item) {
                if (is_array($item)) {
                    ...
                    $items[] = BankResponseDto::fromArray($item);
                }
            }
        }
