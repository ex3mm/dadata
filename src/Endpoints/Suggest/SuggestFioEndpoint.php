...
/** Endpoint для получения подсказок по ФИО */
final class SuggestFioEndpoint extends AbstractEndpoint
{
    protected function getPath(): string
    {
        return '/suggestions/api/' . self::API_VERSION . '/rs/suggest/fio';
    }
        ...
        if (is_array($suggestionsData)) {
            foreach ($suggestionsData as $item) {
                if (is_array($item)) {
                    ...
                    $items[] = FioSuggestionResponseDto::fromArray($item);
                }
            }
        }
